...
export async function GET(request: Request) {
  ...
  for (const [childId, parentId] of childToParent) {
    const child = columnAmounts.get(childId);
    if (!child) continue;
    const parent = columnAmounts.get(parentId) ?? { netChange: {}, endingBalance: {} };
    ...
    columnAmounts.set(parentId, parent);
    columnAmounts.delete(childId);
  }
